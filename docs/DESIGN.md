...
### 实测（小型 PR）
- `PyGithub#2900`（1 行文档改动）：**594 输入 + 61 输出 token**，端到端约 **2.0s**（含 GitHub 拉取 + DeepSeek 调用）。
- token 用量随 diff 体积线性增长；上述为下限参考。

### 估算（较大 PR，接近上限）
- 接近大小上限（约 20000 行变更）的 PR，单次审查约 **~30K 输入 + ~2K 输出 token**（估算值，非实测）。

### 成本对比（按 ~30K 输入 + ~2K 输出估算）

| 模型 | 假设单价（输入 / 输出，每百万 token） | 单次估算成本 |
| --- | --- | --- |
| DeepSeek-V3 | ≈ ¥1 / ¥2 | **≈ ¥0.05** |
| Claude Sonnet | ≈ ¥21 / ¥108（$3 / $15，按 ¥7.2/$ 折算） | **≈ ¥0.86** |

> 单价随官方调整与缓存命中而变化，以上为量级估算。结论：**DeepSeek 单次成本比 Claude 低约一个数量级**（视输入/输出比例与缓存，约 15×~50×）——这是默认选 DeepSeek 的核心理由。
...
- **大 PR 限流**：`> 50 文件` 或 `> 20000 行变更` 直接拒绝（HTTP 422），防止单请求 token 滥用与超时；配合重试退避避免对上游放大压力。
